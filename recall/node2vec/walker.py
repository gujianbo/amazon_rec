...
from partition import partition_num
...
class RandomWalker:
    ...
    def deepwalk_walk(self, walk_length, start_node, fdout):
    ...
    def node2vec_walk2(self, walk_length, start_node, fdout):
        """
        Reference:
    ...
    def simulate_walks(self, num_walks, walk_length, workers=1, verbose=0, sentence_file=""):
        G = self.G
        nodes = list(G.nodes())

        results = Parallel(n_jobs=workers, verbose=verbose, )(
            delayed(self._simulate_walks)(nodes, num, walk_length, sentence_file) for num in
    ...
    def _simulate_walks(self, nodes, num_walks, walk_length, sentence_file):
        # walks = []
        fdout = open(sentence_file, "w")
        for rnd in range(num_walks):
            random.shuffle(nodes)
            for v in tqdm(nodes, "walk in round "+str(rnd)):
                if self.p == 1 and self.q == 1:
                    self.deepwalk_walk(
                        walk_length=walk_length, start_node=v, fdout=fdout)
    ...
    def preprocess_transition_probs(self):
        ...
        G = self.G
        alias_nodes = {}
        for node in tqdm(G.nodes(), "preprocess_transition_probs"):
