...
nohup python -u get_candidation.py --input_file ${root}/data/sessions_train.csv \
  --output_file ${root}/cand/candi_100_top100.csv --single_topk 100 \
...
nohup python -u flatten_candidation.py --input_file ${root}/cand/candi_100_top100.csv \
  --output_file ${root}/cand/candi_100_top100.flatten.csv \
  --log_file ${root}/log/flatten.log > flatten.log 2>&1 &

# 测试
nohup python -u flatten_candidation.py --input_file ${root}/cand/candi_100_top100.csv \
  --output_file ${root}/cand/candi_100_top100.test.flatten.csv \
  --log_file ${root}/log/flatten.log --neg_sample_rate 0 \
  --if_hash_sample 1 --sample_cnt 20 > flatten.log 2>&1 &

# 提交
nohup python -u flatten_candidation.py --input_file ${root}/cand/submission_100_top100.csv \
  --output_file ${root}/cand/submission_100_top100.flatten.csv \
