...
model = AttentionU2I(
    ...
    num_country=7,
    num_layers=config.tfm_num_layers,
...
train_dataset = TrainDataset(file=config.train_file, candi_buffer_size=128, max_seq_len=config.max_seq_len)
train_dataloader = DataLoader(train_dataset, config.train_batch_size)
test_dataset = TestDataset(file=config.test_file, candi_buffer_size=128, max_seq_len=config.max_seq_len)
